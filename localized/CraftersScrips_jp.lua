...
configs:
  クラフタークラス:
    description: 納品・作成に使用するクラフター職を選択します。
    is_choice: true
  ...
  ArtisanリストID:
    default: 1
    description: 中間素材を作成するための Artisan のリストID。
  ...
  薬:
    default: false
    description: 極錬精薬HQを使用する（HQのみ対応）。
  ...
  インベントリ空き数設定:
    default: 5
    description: 作成／納品を開始するために必要な最小インベントリ空き数。

  スカイスチールツール解放済みかどうか:
    default: true
    description: スカイスチールツール解放済みかどうか（スクリップ交換メニュー番号補正に使用）。
...
CrafterClass          = Config.Get("クラフタークラス")
ScripColor            = Config.Get("入手するスクリップ")
ArtisanListId         = Config.Get("ArtisanリストID")
ItemToBuy             = Config.Get("交換アイテム")
HomeCommand           = Config.Get("どこで製作などをするか")
HubCity               = Config.Get("拠点をどこにするか")
Potion                = Config.Get("薬")
Retainers             = Config.Get("AutoRetainer設定")
GrandCompanyTurnIn    = Config.Get("GC納品（要Deliveroo）")
MinInventoryFreeSlots = Config.Get("インベントリ空き数設定")
SkystellToolsUnlocked = Config.Get("スカイスチールツール解放済みかどうか")
...
ClassList = {
    crp = { classId =  8, className = "木工師"      },
    bsm = { classId =  9, className = "鍛冶師"     },
    arm = { classId = 10, className = "甲冑師"        },
    gsm = { classId = 11, className = "彫金師"      },
    ltw = { classId = 12, className = "革細工師"  },
    wvr = { classId = 13, className = "裁縫師"         },
    alc = { classId = 14, className = "錬金術師"      },
    cul = { classId = 15, className = "調理師"     }
}
...
function ScripExchange()
    ...
    elseif Addons.GetAddon("InclusionShop").Ready then
        ...
        local scripExchange = Entity.GetEntityByName("スクリップ取引窓口")
...
function ProcessRetainers()
    ...
    if not Dalamud.Log("[CraftersScrips] check retainers ready") and not IPC.AutoRetainer.AreAnyRetainersAvailableForCurrentChara() or Inventory.GetFreeInventorySlots() <= 1 then
        ...
        local summoningBell = Entity.GetEntityByName("呼び鈴")
...
elseif not AtInn and Svc.ClientState.TerritoryType ~= 1186 then
    IPC.Lifestream.ExecuteCommand("ネクサスアーケード")
